...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!ADZUNA_APP_ID || !ADZUNA_API_KEY) {
      console.error('Missing credentials - ADZUNA_APP_ID:', !!ADZUNA_APP_ID, 'ADZUNA_API_KEY:', !!ADZUNA_API_KEY)
      return new Response(JSON.stringify({ 
        error: 'Adzuna credentials not configured. Please set ADZUNA_APP_ID and ADZUNA_API_KEY as Edge Function secrets.',
        results: [],
        count: 0
      }), {
    ...
    const { query, location, radius, salary_min, salary_max, remote_only } = body
    
    console.log('Search request:', JSON.stringify({ query, location, radius, remote_only }))

    const baseUrl = 'https://api.adzuna.com/v1/api/jobs/ca/search/1'
    ...
    if (remote_only) url.searchParams.set('what', query ? `${query} remote` : 'remote')
    ...
    if (!response.ok) {
      console.error('Adzuna API error:', response.status, responseText.substring(0, 500))
      
      // Return a helpful error but with 200 status so client can read it
      return new Response(JSON.stringify({ 
        error: `Adzuna API error (${response.status}). This usually means invalid API credentials. Please verify your ADZUNA_APP_ID and ADZUNA_API_KEY secrets.`,
    ...
    try {
      ...
      console.error('Failed to parse Adzuna response:', responseText.substring(0, 500))
      return new Response(JSON.stringify({ 
